...
function home() {
  ...
  buttonA.innerHTML = "Add Notes";
  buttonA.addEventListener("click", function () {
    renderPage("addNotes");
  });
  buttonB.innerHTML = "Display Notes";
...
function addNotes() {
  ...
  text.id = "inputNote";
  text.placeholder = "write notes here.more than a character please";
  ...
  noteInsight.innerHTML =
    "write Importance as a number, 1-5.";
  ...
  //checking if importance is a number

  document.body.querySelector(".button").addEventListener("click", function () {
    if (inputValid(inputNote) && numberValid(inputImp)) {
      submission();
      window.alert(
        "A number! note submitted."
      );
      addNotes();
    } else {
      if (!inputValid(inputNote) && numberValid(inputImp)) {
        window.alert(
          "You did not type a note."
        );
      } else if (inputValid(inputNote) && !numberValid(inputImp)) {
        window.alert("choose an importance value. didn't choose one");
      } else {
        window.alert(
          "type something."
        );
